  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        const receiver = await User.findById(followingId).select("fcmTokens");
        ...
        if (receiver && receiver.fcmTokens && receiver.fcmTokens.length > 0) {
          await sendToTokens(
            receiver.fcmTokens,
            { title: "New Follower", body: `${senderName} started following you` },
            { type: "user", id: String(followerId), clickAction: `/profile/${followerId}` }
          );
        }
...
/**
 * Retrieves the users that are following the current user.
 *
...
const getFollowers = async (req, res) => {
  try {
    const relationships = await Relationship.find({
      ...
      .populate("follower", "_id name avatar location")
      .lean();

    const followers = relationships
      .map((relationship) => ({
        ...relationship.follower,
        followedSince: relationship.createdAt,
      }))
      .sort((a, b) => b.followedSince - a.followedSince);

    res.status(200).json(followers);
  } catch (error) {
    res.status(500).json({
      message: "Some error occurred while retrieving the followers",
    });
  }
};
