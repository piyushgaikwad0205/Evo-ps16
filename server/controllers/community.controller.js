...
const getCommunities = async (req, res) => {
  try {
    ...
    const communities = await Community.find({ collegeId: user.collegeId });
...
const getCommunity = async (req, res) => {
  try {
    ...
    if (!user.collegeId || (community.collegeId && community.collegeId.toString() !== user.collegeId.toString())) {
      return res.status(403).json({ message: "Access denied. Community belongs to another college." });
    }
...
const createCommunity = async (req, res) => {
  try {
    // Check for Admin Auth (populated by requireAdminAuth middleware)
    if (!req.admin || req.adminRole !== 'superadmin') {
      return res.status(403).json({ message: "Only Super Admin can create communities." });
    }

    const communitiesData = Array.isArray(req.body) ? req.body : [req.body];

    // Validate that collegeId is present and valid for each community if not superadmin
    for (const c of communitiesData) {
      if (req.adminRole !== 'superadmin' && !c.collegeId) {
        return res.status(400).json({ message: "collegeId is required." });
      }
    }

    const savedCommunities = await Community.insertMany(communitiesData);
