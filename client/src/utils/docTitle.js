...
export const getTitleFromRoute = (path) => {
  if (titleMap[path]) {
    return `${titleMap[path]} | Campus Connects`;
  }
  ...
  if (userProfileRegex.test(path)) {
    return "User Profile | Campus Connects";
  } else if (postRegex.test(path)) {
    return "Post | Campus Connects";
  } else if (communityRegex.test(path)) {
    return "Community | Campus Connects";
  }

  return "Campus Connects";
};
