...
const HODDashboard = () => {
    ...
    const stats = [
        ...
        { label: "Employee ID", value: user.employeeId, icon: HiUser, color: "purple" },
        { label: "Experience", value: `${user.experience} years`, icon: HiChartBar, color: "green" },
        { label: "Status", value: user.isActive ? "Active" : "Inactive", icon: HiAcademicCap, color: user.isActive ? "green" : "red" },
    ];
                ...
                {activeTab === "overview" ? (
                    <>
                        ...
                        >
                            ...
                            >
                                <HiUsers className={`w-8 h-8 mb-3 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`} />
                                <h3 className={`font-semibold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Classes & Sections</h3>
                                <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>View classes and assign teachers</p>
                            </div>
                            <div className={`p-6 rounded-2xl shadow-sm border ${isDarkMode ? "bg-dark-bg-secondary border-white/5" : "bg-white border-gray-200"} hover:shadow-lg transition-shadow cursor-pointer`}>
                                <HiCalendar className={`w-8 h-8 mb-3 ${isDarkMode ? "text-purple-400" : "text-purple-600"}`} />
                                <h3 className={`font-semibold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Schedule</h3>
                                <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>View and manage class schedules</p>
                            </div>
                            <div className={`p-6 rounded-2xl shadow-sm border ${isDarkMode ? "bg-dark-bg-secondary border-white/5" : "bg-white border-gray-200"} hover:shadow-lg transition-shadow cursor-pointer`}>
                                <HiChartBar className={`w-8 h-8 mb-3 ${isDarkMode ? "text-green-400" : "text-green-600"}`} />
                                <h3 className={`font-semibold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Reports</h3>
                                <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>View department performance reports</p>
                            </div>
                        </motion.div>
                    </>
                ) : (
                    <ClassManagement role="hod" departmentId={user.department?._id} />
                )}
            </main>
        </div>
    );
