...
export const getPosts = async (limit = 10, skip = 0, type = "following") => {
  try {
    const { data } = await API.get(`/posts?limit=${limit}&skip=${skip}&type=${type}`);
    return { error: null, data };
  } catch (error) {
    return handleApiError(error);
  }
...
export const deleteComment = async (id, commentId) => {
  try {
    const { data } = await API.delete(`/posts/${id}/comment/${commentId}`);
    return { error: null, data };
  } catch (error) {
    return handleApiError(error);
  }
