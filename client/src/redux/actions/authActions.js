...
export const initializeAuth = () => async (dispatch) => {
  ...
  if (accessToken && refreshToken) {
    // Always hydrate user/session from storage first so user remains logged in
    dispatch(setAccessToken(accessToken));
    dispatch(setRefreshToken(refreshToken));
    dispatch(setUserData(stored?.user));
    // If token expired, try to refresh in background without logging out
    if (!isValidToken(accessToken)) {
      try { await dispatch(refreshTokenAction(refreshToken)); } catch { }
    }
  }
};
